           ...
           // SwiftyDB.addObject(adsf)
            ...
            context("object is added") {
                it("should contain the object after it is added") {
                    ...
                    expect(database.dataFor(TestClass(), matchingFilter: filter).value?.count) == 1
                }
            }
            ...
            context("object is deleted") {
                it("should not contain the object after deletion") {
                    ...
                    expect(database.dataFor(TestClass(), matchingFilter: filter).value?.count) == 0
                }
            }
            ...
            context("object is updated") {
                it("should update existing objects") {
                    ...
                    if let stringValue = database.dataFor(TestClass(), matchingFilter: filter).value?.first?["string"] as? String {
